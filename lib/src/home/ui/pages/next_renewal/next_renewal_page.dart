...
class NextRenewalPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<NextRenewalController>(
      ...
      builder: (controller) => RefreshIndicator(
        onRefresh: controller.getRenewals,
        child: Scaffold(
          appBar: appBarWidget(
  ...
  Widget _itemRenewal(DashboardQuotaResponse quota) {
    ...
    return QuotaOfCalendarWidget(
      ...
      amount: quota.amount,
      detail: quota.dateToPay.formatDMMYYY(),
      stateWidget: _stateWidget(quota),
    );
  }

  Widget _stateWidget(DashboardQuotaResponse quota) {

    String currentAmount = 'S/ ${quota.amountOfLoan?.formatDecimals()}';
    String lastAmount = 'S/ ${quota.amountOfLastLoan?.formatDecimals()}';

    return Column(
      children: [
        TagWidget(
          backgroundColor: Colors.white,
          textColorAndIcon: successColor(),
          title: currentAmount, icon: Icons.punch_clock_rounded),
        TagWidget(
          backgroundColor: Colors.white,
